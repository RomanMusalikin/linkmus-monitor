...
export default function Dashboard() {
  ...
  const avgCPU = online.length > 0
    ? Math.round(online.reduce((s, n) => s + n.cpu, 0) / online.length)
    : 0;

  const avgRAM = online.length > 0
    ? Math.round(online.reduce((s, n) => s + (n.ramTotal > 0 ? (n.ramUsed / n.ramTotal) * 100 : 0), 0) / online.length)
    : 0;

  const avgDisk = online.length > 0
    ? Math.round(online.reduce((s, n) => s + (n.diskUsage || 0), 0) / online.length)
    : 0;
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="text-xs text-slate-500">{total} всего</span>
      </div>
