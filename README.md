...
- **Дашборд** — карточки всех узлов с CPU, RAM, диском, сетью и статусом сервисов; drag-and-drop сортировка, переименование узлов (псевдонимы)
- **Детальная страница узла** — графики, процессы, диски, сетевые интерфейсы, TCP, SNMP, FSRM-квоты (реальные размеры через WMI)
- **История** — 24ч с 10-минутным усреднением; долгосрочная история 7д / 14д / 30д по часовым агрегатам
- **Пробы сервисов** — SSH, RDP, SMB, HTTP, WinRM, DNS с временем отклика (server-side TCP)
- **Экспорт** — выгрузка метрик узла в CSV или JSON прямо из браузера
...
mon server delete   # полностью удалить сервер (служба, файлы, шим)
mon agent  delete   # полностью удалить агент (служба, файлы, шим)
mon help
```
...
| Windows | статус RDP/SMB через WMI, FSRM-квоты (путь, лимит, использование, % заполнения, нарушения за 24ч) |

---

## Пробы сервисов (server-side)

Сервер периодически опрашивает каждый узел по его IP. Все пробы выполняются **на стороне сервера** — агент для этого не нужен.

### TCP-пробы

Метод одинаковый для всех сервисов: `net.DialTimeout("tcp", "IP:порт", 3s)`. Если TCP-соединение установлено — статус зелёный, если таймаут или отказ — красный. Полный протокольный handshake не выполняется: проверяется только сетевая доступность порта.

| Сервис | Порт | Что проверяется |
|--------|------|-----------------|
| SSH | 22 | TCP-порт открыт (SSH-демон слушает) |
| RDP | 3389 | TCP-порт открыт (показывается только для Windows-узлов) |
| SMB | 445 | TCP-порт открыт (Windows — показывается только для Windows-узлов) |
| HTTP | 80 | TCP-порт открыт (любой веб-сервер) |
| WinRM | 5985 | TCP-порт открыт (Windows Remote Management; показывается только для Windows) |
| DNS | 53 | TCP-порт открыт (DNS over TCP) |

Время отклика — это время от начала dial до успешного установления соединения (в мс). Значение `<1 мс` означает соединение быстрее 1 мс.

### SNMP-поллер

Опрос по **UDP, порт 161**, community string `public`, протокол SNMPv2c. Выполняется каждые 30 секунд. Собираемые OID:

| OID | Значение |
|-----|----------|
| `1.3.6.1.2.1.1.3.0` (sysUpTime) | Время работы устройства |
| `1.3.6.1.2.1.1.5.0` (sysName) | Имя хоста по SNMP |
| `1.3.6.1.2.1.25.3.3.1.2.1` (hrProcessorLoad) | Загрузка CPU по SNMP |
| `1.3.6.1.2.1.2.1.0` (ifNumber) | Количество сетевых интерфейсов |
| `1.3.6.1.2.1.2.2.1.*` (ifTable) | Трафик по интерфейсам: имя, скорость линка, входящий/исходящий байт/с |

Трафик по интерфейсам вычисляется через BulkWalk по `ifInOctets`/`ifOutOctets` — дельта между двумя опросами. Значения появляются начиная со второго опроса (~30 сек после старта).

Если SNMP недоступен или community не совпадает — поля в интерфейсе остаются пустыми.
...
| GET | `/api/version` | Версия сервера | Нет |
...
| PUT | `/api/nodes/{name}/alias` | Задать псевдоним узла (пустая строка — сбросить) | Bearer токен |
| GET | `/api/history/{name}?range=7d\|14d\|30d` | Долгосрочная история (часовые агрегаты) | Bearer токен |
...
│       ├── history.go             # hourly aggregator, долгосрочная история, HandleNodeHistory
