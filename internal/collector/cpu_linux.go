...
// CollectCPUBreakdown возвращает user%, system%, iowait%, steal%, total% через дельту между вызовами.
func CollectCPUBreakdown() (user, system, iowait, steal, total float64) {
	...
	if len(prevCPUTimes) > 0 {
		...
		if dt > 0 {
			...
			iowait = clamp((curr[0].Iowait - prevCPUTimes[0].Iowait) / dt * 100)
			steal = clamp((curr[0].Steal - prevCPUTimes[0].Steal) / dt * 100)
		}
	}
	prevCPUTimes = curr
	return
